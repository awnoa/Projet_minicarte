...
public class ViewApp extends JFrame {
	...
	private final int CATEGORIE = 0;
	private final int LIEU = 1;
	private final int NOM = 2;
	...
	public HashSet<Integer> addCritereCategorie(String key){

		Set<Integer> keys = model.getData().keySet();
		HashSet<Integer> ret = new HashSet<>();
		PointInteret pt;
		for (Integer i : keys) {
			pt = model.getData().get(i);			
			if(key.equals(pt.getCategorie())){
				ret.add(i);
			}
		}
		return ret;
	}
	public void removeCritereCategorie(String key){
		PointInteret pt;

		for (java.util.Iterator<Integer> iterator = model.idCritereCategories.iterator(); iterator.hasNext(); ) {
	...
	public HashSet<Integer> displayAllCategorie(){
		HashSet<Integer> ids = new HashSet<>();
		for (JCheckBox cb : categories) {
			if (! cb.isSelected()) {
				cb.setSelected(true);
				ids.addAll(addCritereCategorie(cb.getText()));
			}
		}
		return ids;
	}
	...
	public void hideAllCategorie() {
		for (JCheckBox cb : categories) {
			if (cb.isSelected()) {
				...
				removeCritereCategorie(cb.getText());
			}
		}
	}
	...
	public void paintIcons() {
		...
		int c = 0;
		labMap.resetPoints();
		for(Integer integer : model.mergeHashsets()){
			pointInteret = model.getData().get(integer);

			String withoutAccent = Normalizer.normalize(pointInteret.getCategorie(), Normalizer.Form.NFD);
			withoutAccent = withoutAccent.replaceAll("[\\p{InCombiningDiacriticalMarks}]", "");
			withoutAccent = withoutAccent.trim();

			ImageIcon img = new ImageIcon(System.getProperty("user.dir")+"/images/"+withoutAccent+".png");
			addIcon(img,pointInteret, pointInteret.getLongitude(), pointInteret.getLatitude());
			c++;
		}
	...
	public HashSet<Integer> addCritereNom(String recherche) {
		Set<Integer> keys = model.getData().keySet();
		...
		for (Integer i : keys) {
			pt = model.getData().get(i);			
			if(pt.getNom().toLowerCase().contains(recherche.toLowerCase())){
				ret.add(i);
			}
		}
		return ret;
	...
	public HashSet<Integer> addCritereLocalisation(float latMax, float latMin, float longMax, float longMin, ArrayList<String> communes ) {
		...
		for (Integer i : keys) {
			pt = model.getData().get(i);			
			if(pt.getLatitude()<=latMax && pt.getLongitude()>=latMin && pt.getLongitude()<=longMax&& pt.getLongitude()>=longMin &&
					communes.contains(pt.getCommune().toLowerCase().trim())){
				ret.add(i);
			}
		}
		return ret;
